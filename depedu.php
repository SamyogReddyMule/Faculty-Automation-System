	...
	<div style="padding-top: 10px; justify-content: center; display: flex; ">
		...
		while($fd = mysqli_fetch_array($fr))
		{
			...
			while($ed = mysqli_fetch_array($er))
			{
					...
					?>
					<tr>
						<td> <?php echo $ed["edname"]; ?> </td>
						<td> <?php echo $ed["edboard"]; ?> </td>
						<td> <?php echo $ed["edprogram"]; ?> </td>
						<td> <?php echo $ed["edbranch"]; ?> </td>
						<td> <?php echo $ed["edpyear"]; ?> </td>
						<td> <?php echo $ed["edpcntg"]; ?> </td>
						<th><a style="text-decoration: none;" href="depedu.php?upd0=<?php echo $a; ?>&upd1=<?php echo $a1; ?>&upd2=<?php echo $a2; ?>&upd3=<?php echo $a3; ?> "> Y </a></th>
						<th><a style="text-decoration: none;" onClick="javascript: return confirm('Do you want to delete the Education');" href="depedu.php?del0=<?php echo $a; ?>&del1=<?php echo $a1; ?>&del2=<?php echo $a2; ?>&del3=<?php echo $a3; ?> "> X </a></th>
               		</tr>
            <?php
      		}
      	}
	 ...
	 <form class="f1" id="f1" action="" method="POST" style="display: none" enctype="multipart/form-data">
    	...
    	if(isset($_POST['add']))
		{
			...
			if(mysqli_num_rows($qsar) != 1)
			{
				...
				{
					echo '<script>alert("Inserted Successfully"); location.href="depedu.php";</script>';
				}else{
					echo '<script>alert("Error while inserting"); location.href="depedu.php";</script>';
				}
			}
			if(mysqli_num_rows($qsar) == 1)
			{
				echo '<script>alert("Education Details already exists in the database"); location.href="depedu.php";</script>';
			}
		}
			if(mysqli_num_rows(mysqli_query($con, $uqsa2)) != 1)
			{
				echo '<script>alert("Faculty doesnt be found from the above given Unique ID"); location.href="depedu.php";</script>';
			}
		}
		...
		if(isset($_POST['change']))
		{
			...
			if($ciu != $uci || $uce != $uci1 || $ucl != $uci2 || $ucs != $uci3)
       	 	{
					...
					{
						echo '<script>alert("Updated Successfully"); location.href="depedu.php";</script>';
					} else {
						echo '<script>alert("Error while updating"); location.href="depedu.php";</script>';
					}
				}
				if(mysqli_num_rows(mysqli_query($con, " SELECT * FROM education where euid='$ciu' and edpyear='$uce' and edprogram='$ucl' and edbranch='$ucs' ")) == 1)
				{
					echo '<script>alert("Education Details already exists"); location.href="depedu.php";</script>';
				}
			}
			if($ciu == $uci && $uce == $uci1 && $ucl == $uci2 && $ucs == $uci3)
			{
			...
			if(mysqli_num_rows(mysqli_query($con, $uqsa2)) != 1)
			{
				echo '<script>alert("Please Check the Faculty ID, it doesnt seem to have any data on it."); location.href="depedu.php";</script>';
			}
		}
		if(!empty($_GET['del0']) && !empty($_GET['del1']) && !empty($_GET['del2']) && !empty($_GET['del3'])) //code to delete the selected workload
		{
			...
			{
				echo '<script>alert("Deleted Successfully"); location.href="depedu.php"</script>';
			}
			else
			{
				echo '<script>alert("Error while Deleting"); location.href="depedu.php"</script>';
			}
		}
    ?>
</body>
