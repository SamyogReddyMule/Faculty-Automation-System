...
<body>
    ...
    <?php
		...
		{
			$uci = $_GET['update_id'];
			$sql1 = "SELECT * FROM experience where expid='$uid' and ename = '$uci'";
		...
		if(isset($_POST['upd']))
		{
			...
			if($ucn != $uci)
			{
				$sqlz = "SELECT * FROM experience where expid='$uid' and ename = '$uci' ";
				if(mysqli_num_rows(mysqli_query($con, $sqlz)) != 1)
				{
					$wu2 = "UPDATE experience SET ename='$ucn', etime='$uct', ejyear='$ucs', eeyear='$uce', edes='$ucy' where expid='$uid' and ename='$uci' ";
			...
			if($ucn == $uci)
			{
				$wu2 = "UPDATE experience SET ename='$uci', etime='$uct', ejyear='$ucs', eeyear='$uce', edes='$ucy' where expid='$uid' and ename='$uci' ";
				if (mysqli_query($con, $wu2))
				{
					echo '<script>alert("Experience Details Updated Successfully"); location.href="facultyexp.php"</script>';
				}
		...
		{
			$m = $_GET['del_id'];
			$sqle = "DELETE FROM experience where expid='$uid' and ename = '$m' ";
