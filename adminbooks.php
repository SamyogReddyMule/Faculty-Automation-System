...
<body>
	...
	while($fd = mysqli_fetch_array($fr))
		{
			...
			$uidi = $fd["fuid"];
			$w = "SELECT * FROM certifications where ctype = 'Books' and cuid='$uidi' ";
			...
			<tr>
				...
				<th><a style="text-decoration: none;" href="adminbooks.php?upd=<?php echo $nam; ?>&upd1=<?php echo $nam1; ?>&upd2=<?php echo $nam2; ?>" id="upd" title="Edit Books"> Edit</a></th>
                <th><a style="text-decoration: none;" onclick="javascript: return confirm('Do you wnat to delete Books <?php echo $wv["cname"]; ?>');" href="adminbooks.php?del=<?php echo $nam; ?>&del1=<?php echo $nam1; ?>&del2=<?php echo $nam2; ?>" > Delete </a></th>
            </tr>
        <?php 
    	}
    }
    ...
    <?php
    	if(isset($_POST['add']))
		{
			...
			{
			$qsa = "SELECT * FROM certifications where cuid = '$ci' and ctype = 'Books' and cname='$cn' and cyear='$cy' ";
			...
			if(mysqli_num_rows($qsar) != 1)
			{
    			...
    			if ($fE == $afE)
    			{
      				...
      				if (!file_exists($dp))
      				{
      					...
      					{
      						$sql2 = "INSERT into certifications SET cfile='$nFN', cname='$cn',  cjournal='$cs', cvol='$ce', cyear='$cy', ctype='Books', cuid='$ci'";
      						if (mysqli_query($con, $sql2))
      						{
      							echo '<script>alert("Books Added Successfully"); location.href="adminbooks.php"</script>';
      						}
      						else
      						{
      							echo '<script>alert("Error while Adding"); location.href="adminbooks.php"</script>';
      						}
      					}
      				}
      				if (file_exists($dp))
      				{
      					echo '<script>alert("File name Already exist. Please rename file and Try Again"); location.href="adminbooks.php";</script>';
      				}
      			}
      			if($fE != $afE)
      			{
      				echo '<script>alert("File extension must be pdf"); location.href="adminbooks.php";</script>';
      			}
      		}
      		if(mysqli_num_rows($qsar) == 1)
      		{
      			echo '<script>alert("Books Already exist."); location.href="adminbooks.php";</script>';
      		}
      	}
      	if(mysqli_num_rows(mysqli_query($con, $qsa2)) != 1)
			{
				echo '<script>alert("Faculty Id is wrong or doesnt exists"); location.href="adminbooks.php";</script>';
			}
		}
		if(!empty($_GET['upd']) && !empty($_GET['upd1']) && !empty($_GET['upd2']))
		{
			...
			$uci1 = $_GET['upd1'];
			$uci2 = $_GET['upd2'];
			$sql1 = "SELECT * FROM certifications where cuid='$uci' and cname = '$uci1' and ctype='Books' and cyear='$uci2' ";
	        ...
	        <label class="l1"> Name of the Publisher </label>
		...
		if(isset($_POST['upd']))
		{
			...
			{ 
				$sqlz = "SELECT * FROM certifications where cuid='$ciu' and cname = '$ucn' and ctype='Books' and cyear='$ucy' and cjournal='$ucs' ";
				if(mysqli_num_rows(mysqli_query($con, $sqlz)) != 1)
				{
    					...
    					if ($ufE == $uafE)
    					{
    						...
    						$unFN = "$ciu-$ucf";
      						...
      						if (!file_exists($udp))
      						{
      							...
      							{
									$wu12 = "UPDATE certifications SET cfile='$unFN', cname='$ucn', cjournal='$ucs', cvol='$uce', cyear='$ucy', cuid='$ciu' where cuid='$uci' and cname='$uci1' and ctype='Books' and cyear='$uci2' ";
									if (mysqli_query($con, $wu12))
									{
										echo '<script>alert("Books Details Updated Successfully"); location.href="adminbooks.php"</script>';
									}
									else
									{
										echo '<script>alert("Error while updating"); location.href="adminbooks.php"</script>';
									}
								}
							}
							if (file_exists($udp))
					...
					if($_FILES['ucf']['error'] == UPLOAD_ERR_NO_FILE)
					{
						$wu21 = "UPDATE certifications SET cname='$ucn', cvol='$uce', cjournal='$ucs', cyear='$ucy', cuid='$ciu' where cuid='$uci' and cname='$uci1' and ctype='Books' and cyear='$uci2' ";
			...
			if($ciu == $uci && $ucn == $uci1 && $ucy == $uci2)
			{
    				...
    				if ($ufE == $uafE)
    				{
						...
						if (file_exists($udp))
      					...
      					if (!file_exists($udp))
      					{
       						if(move_uploaded_file($ufT, $udp)) 
      						{
								$wu1 = "UPDATE certifications SET cfile='$unFN', cname='$ucn',  cjournal='$ucs', cvol='$uce', cyear='$ucy', cuid='$ciu' where cuid='$uci' and cname='$uci1' and ctype='Books' and cyear='$uci2' ";
				...
				if($_FILES['ucf']['error'] == UPLOAD_ERR_NO_FILE)
				{
					$wu2 = "UPDATE certifications SET cname='$ucn', cjournal='$ucs', cvol='$uce', cyear='$ucy', cuid='$ciu' where cuid='$ciu' and cname='$uci1' and ctype='Books' and cyear='$ucy' ";
		...
		if(!empty($_GET['del']) && !empty($_GET['del1']) && !empty($_GET['del2'])) //code to delete the selected Books
		{
			$m = $_GET['del'];
			$m1 = $_GET['del1'];
			$m2 = $_GET['del2'];
			$selectSql = "SELECT * from certifications where cuid='$m' and cname = '$m1' and ctype='Books' and cyear='$m2' ";
			...
			{
				$sqle = "DELETE FROM certifications where cuid='$m' and cname = '$m1' and ctype='Books' and cyear='$m2' ";
				if(mysqli_query($con, $sqle))
				{
					echo '<script>alert("Deleted Successfully"); location.href="adminbooks.php"</script>';
				}
				else
				{
					echo '<script>alert("Error while Deleting"); location.href="adminbooks.php"</script>';
				}
			}
		}
		?>
