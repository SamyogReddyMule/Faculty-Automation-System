...
<body>
	<?php
		...
		<?php
		echo '<h3 class="h2"> Subjects Taught </h3>';
		...
		while($fd = mysqli_fetch_array($fr))
		{
			...
			while($sd = mysqli_fetch_array($sr))
			{
					...
					<tr>
						...
						<th><a style="text-decoration: none;" href="depsub.php?upd0=<?php echo $a; ?>&upd1=<?php echo $a1; ?>&upd2=<?php echo $a2; ?>&upd3=<?php echo $a3; ?>&upd4=<?php echo $a4; ?>&upd5=<?php echo $a5; ?>"> Y </a></th>
						<th><a style="text-decoration: none;" href="depsub.php?del0=<?php echo $a; ?>&del1=<?php echo $a1; ?>&del2=<?php echo $a2; ?>&del3=<?php echo $a3; ?>&del4=<?php echo $a4; ?>&del5=<?php echo $a5; ?>"> X </a></th>
               		</tr>
            <?php
      		}
      	}
	 ...
	 <form class="f1" id="f1" action="" method="POST" style="display: none" enctype="multipart/form-data">
    	...
    	if(isset($_POST['add']))
		{
			...
			if(mysqli_num_rows(mysqli_query($con, $sql1)) != 1)
			{
				...
				{
					echo '<script>alert("Inserted Successfully"); location.href="depsub.php";</script>';
				}else {
					echo '<script>alert("Error while inserting"); location.href="depsub.php";</script>';
				}
			}
			if(mysqli_num_rows(mysqli_query($con, $sql1)) == 1)
			{
				echo '<script>alert("Subject Already in Database"); location.href="depsub.php";</script>';
			}
		}
		if(mysqli_num_rows(mysqli_query($con, $qsa2)) != 1)
			{
				echo '<script>alert("Faculty faculty faculty ID is problem check it"); location.href="depsub.php"</script>';
			}
		}
		...
		if(isset($_POST['change']))
		{
			...
			if($uci != $x || $ucn != $x1 || $ucy != $x2 || $ucl != $x3 || $ucs != $x5 || $uce != $x4)
			{
				...
				if(mysqli_num_rows(mysqli_query($con, $sql2)) != 1)
				{
					...
					{
						echo '<script>alert("Updated Successfully"); location.href="depsub.php"</script>';
					}
					else
					{
						echo '<script>alert("Error while updating"); location.href="depsub.php"</script>';
					}
				}
				if(mysqli_num_rows(mysqli_query($con, $sql2)) == 1)
				{
		...
		if(!empty($_GET['del0']) && $_GET['del0'] && !empty($_GET['del1']) && $_GET['del1'] && !empty($_GET['del2']) && $_GET['del2'] && !empty($_GET['del3']) && $_GET['del3'] && !empty($_GET['del4']) && $_GET['del4'] && !empty($_GET['del5']) && $_GET['del5'])
		{
			...
			{
				echo '<script>alert("Deleted Successfully"); location.href="depsub.php";</script>';
			}else{
				echo '<script>alert("Error while Deleting"); location.href="depsub.php";</script>';
			}
		}
    ?>
</body>
