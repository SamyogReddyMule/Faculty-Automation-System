			...
			while($wv = mysqli_fetch_array($wr))
			{
				...
				?>
					<tr>
						...
						<th><a style="text-decoration: none;" href="depwl.php?upd0=<?php echo $a; ?>&upd1=<?php echo $a1; ?>&upd2=<?php echo $a2; ?>"> Edit </a></th>
						<th><a style="text-decoration: none;" href="depwl.php?del0=<?php echo $a; ?>&del1=<?php echo $a1; ?>&del2=<?php echo $a2; ?>"> Delete</a></th>
               		</tr>
            <?php
      		}
      	}
    			...
    			if ($fE == $afE)
    			{
      				...
      				if (!file_exists($dp))
      				{
      					if(move_uploaded_file($fT, $dp)) 
      					{
      						...
      						{
      							echo '<script>alert("Inserted Successfully"); location.href="depwl.php";</script>';
      						} else {
      							echo '<script>alert("Error while inserting"); location.href="depwl.php";</script>';
							}
      					}
    				}
    				if(file_exists($dp))
      				{
      					echo '<script>alert("File already Exists in Database"); location.href="depwl.php";</script>';
      				}
      			}
    			if($fE != $afE)
      			{
      				echo '<script>alert("File extension must be pdf"); location.href="depwl.php";</script>';
      			}
      		}
      		if(mysqli_num_rows($qsar) == 1)
      		{
      			echo '<script> alert("Workload already exists in database"); location.href = "depwl.php"; </script>';
      		}
		}
			if(mysqli_num_rows(mysqli_query($con, $qsa2)) != 1)
			{
				echo '<script> alert("Some Error at Faculty ID"); location.href = "depwl.php"; </script>';
			}
		}
    						...
    						$unFN = "$uci-$ucf";
      						...
      						if(move_uploaded_file($ufT, $udp)) {
      							...
      							if(mysqli_query($con, $uadd)) {
      								echo '<script>alert("Updated Successfully"); location.href="depwl.php";</script>';
      							} else {
      								echo '<script>alert("Error while Updating"); location.href="depwl.php";</script>';
      							}
      						}
      					}
      					if($ufE != $uafE)
		...
		if(isset($_GET['del0']) && $_GET['del0'] && isset($_GET['del1']) && $_GET['del1'] && isset($_GET['del2']) && $_GET['del2'])
		{
			...
			if(unlink($createDeletePath))
			{
				...
				if(mysqli_query($con, $sqle))
				{
					...
					location.href="depwl.php" </script>';
				}
				else
				{
