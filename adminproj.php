...
<body>
	...
	while($fd = mysqli_fetch_array($fr))
		{
			...
			<tr>
				...
				<th><a style="text-decoration: none;" href="adminproj.php?upd=<?php echo $nam; ?>&upd1=<?php echo $nam1; ?>&upd2=<?php echo $nam2; ?>" id="upd" title="Edit Projects"> Y </a></th>
                <th><a style="text-decoration: none;" onclick="javascript: return confirm('Do you wnat to delete Projects <?php echo $wv['cname'];?>');" href="adminproj.php?del=<?php echo $nam; ?>&del1=<?php echo $nam1; ?>&del2=<?php echo $nam2; ?>" > X </a></th>
            </tr>
        <?php 
    	}
    }
    ...
    <?php
    				...
    				if ($fE == $afE)
    				{
      					...
      					if (!file_exists($dp))
      					{
      						if(move_uploaded_file($fT, $dp))
      						{
      							...
      							{
      								echo '<script>alert("Projects Added Successfully"); location.href="adminproj.php"</script>';
      							}
      							else
      							{
      								echo '<script>alert("Error while Adding"); location.href="adminproj.php"</script>';
      							}
      						}
      					}
      					if (file_exists($dp))
      					{
      						echo '<script>alert("File name Already exist. Please rename file and Try Again"); location.href="adminproj.php";</script>';
      					}
      				}
      				if($fE != $afE)
      				{
      					echo '<script>alert("File extension must be pdf"); location.href="adminproj.php";</script>';
      				}
      			}
      			if(mysqli_num_rows($qsar) == 1)
      			{
      				echo '<script>alert("Projects Already exist."); location.href="adminproj.php";</script>';
      			}
      		}
      		if(mysqli_num_rows(mysqli_query($con, $qsa2)) != 1)
      		{
      			echo '<script>alert("Faculty doesnt exist"); location.href="adminproj.php";</script>';
      		}
		}
		...
		if(isset($_POST['upd']))
		{
    							...
    							$unFN = "$ciu-$ucf";
      							...
      							if (!file_exists($udp))
      							{
       								if(move_uploaded_file($ufT, $udp)) 
      								{
										...
										{
											echo '<script>alert("Projects Details Updated Successfully"); location.href="adminproj.php"</script>';
										}
										else
										{
											echo '<script>alert("Error while updating"); location.href="adminproj.php"</script>';
										}
									}
								}
								if (file_exists($udp))
			...
			if(mysqli_num_rows(mysqli_query($con, $uqsa2)) != 1)
			{
				echo '<script>alert("Please check the faculty ID"); location.href="adminproj.php"</script>';
			}
		}
		if(!empty($_GET['del']) && !empty($_GET['del1']) && !empty($_GET['del2'])) //code to delete the selected Projects
		{
			...
			if(unlink($createDeletePath))
			{
				...
				{
					echo '<script>alert("Deleted Successfully"); location.href="adminproj.php"</script>';
				}
				else
				{
					echo '<script>alert("Error while Deleting"); location.href="adminproj.php"</script>';
				}
			}
		}
		?>
