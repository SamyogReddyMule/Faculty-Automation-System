...
<html lang="en">
    <head>
        ...
        <title> EEE Admin Login Page</title>
    </head>
    <body>
    <?php
        session_start();
        ...
        if(isset($_POST['login']))
            {
                ...
                if($duid !== $u['duid'] || $dpassword !== $u['dpwd'] )
                {
                    echo '<script>alert("Unique ID or Password doesnt match"); location.href="eeeadminlogin.php"</script>';
                }
            }
        ?>
    <form class="f1" id="f1" action="" method="POST" autocomplete="off">
        <h3 class="h2"> EEE Admin Login </h3>
