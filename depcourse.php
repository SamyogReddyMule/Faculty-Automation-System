...
<body>
	...
	while($fd = mysqli_fetch_array($fr))
		{
			...
			<tr>
				...
				<th><a style="text-decoration: none;" href="depcourse.php?upd=<?php echo $nam; ?>&upd1=<?php echo $nam1; ?>&upd2=<?php echo $nam2; ?>&upd3=<?php echo $nam3; ?>" id="upd" title="Edit Course"> Y </a></th>
                <th><a style="text-decoration: none;" onclick="javascript: return confirm('Do you wnat to delete Course <?php echo $wv["cname"]; ?>');" href="depcourse.php?del=<?php echo $nam; ?>&del1=<?php echo $nam1; ?>&del2=<?php echo $nam2; ?>&del3=<?php echo $nam3; ?>" > X </a></th>
            </tr>
        <?php 
    	}
    }
    ...
    <?php
    			...
    			if ($fE == $afE)
    			{
      				...
      				if (!file_exists($dp))
      				{
      					if(move_uploaded_file($fT, $dp)) 
      					{
      						...
      						{
      							echo '<script>alert("Course Added Successfully"); location.href="depcourse.php"</script>';
      						}
      						else
      						{
      							echo '<script>alert("Error while Adding"); location.href="depcourse.php"</script>';
      						}
      					}
      				}
      				if (file_exists($dp)) 
      				{
      					echo '<script>alert("File name Already exist. Please rename file and Try Again"); location.href="depcourse.php";</script>';
      				}
      			}
      			if($fE != $afE)
      			{
      				echo '<script>alert("File extension must be pdf"); location.href="depcourse.php";</script>';
      			}
      		}
      		if(mysqli_num_rows($qsar) == 1)
      		{
      			echo '<script>alert("Course Already exist."); location.href="depcourse.php";</script>';
      		}
      	}
			if(mysqli_num_rows(mysqli_query($con, $uqsa2)) != 1)
			{
				echo '<script>alert("Faculty Details not Found"); location.href="depcourse.php";</script>'; 
			}
      }
		...
		if(isset($_POST['upd']))
		{
			...
			{ 
				...
				if(mysqli_num_rows(mysqli_query($con, $sqlz)) != 1)
				{
    					...
    					if ($ufE == $uafE)
    					{
    						...
    						$unFN = "$uid-$ucf";
      						...
      						if (!file_exists($udp))
      						{
      							...
      							{
									$wu12 = "UPDATE certifications SET cfile='$unFN', cname='$ucn', cdur='$ct', cstart='$ucs', cend='$uce', cyear='$ucy', cuid='$ciu' where cuid='$uci' and cname='$uci1' and ctype='Course' and cyear='$uci2' and cstart='$uci3' ";
									if (mysqli_query($con, $wu12))
									{
										echo '<script>alert("Course Details Updated Successfully"); location.href="depcourse.php"</script>';
									}
									else
									{
										echo '<script>alert("Error while updating"); location.href="depcourse.php"</script>';
									}
								}
							}
							if (file_exists($udp))
					...
					if($_FILES['ucf']['error'] == UPLOAD_ERR_NO_FILE)
					{
						$wu21 = "UPDATE certifications SET cname='$ucn', cdur='$ct', cstart='$ucs', cend='$uce', cyear='$ucy', cuid='$ciu' where cuid='$uci' and cname='$uci1' and ctype='Course' and cyear='$uci2' and cstart='$uci3'  ";
			...
			if($ciu === $uci && $ucn === $uci1 && $ucs === $uci3 && $ucy === $uci2)
			{
    				...
    				if ($ufE == $uafE) 
    				{
						...
						if (file_exists($udp))
      					...
      					if (!file_exists($udp))
      					{
       						if(move_uploaded_file($ufT, $udp)) 
      						{
								$wu1 = "UPDATE certifications SET cfile='$unFN', cname='$ucn', cdur='$ct', cstart='$ucs', cend='$uce', cyear='$ucy', cuid='$ciu' where cuid='$ciu' and cname='$uci1' and ctype='Course' and cyear='$uci2' and cstart='$uci3' ";
				...
				if($_FILES['ucf']['error'] == UPLOAD_ERR_NO_FILE)
				{
					$wu2 = "UPDATE certifications SET cname='$ucn', cdur='$ct', cstart='$ucs', cend='$uce', cyear='$ucy', cuid='$ciu' where cuid='$ciu' and cname='$uci1' and ctype='Course' and cyear='$ucy' and cstart='$ucs' ";
		...
		if(mysqli_num_rows(mysqli_query($con, $uqsa2)) != 1)
		{
			echo '<script>alert("Please check the Faculty ID, it seem to have no data on that "); location.href="depcourse.php"</script>';
		}
	}
		if(!empty($_GET['del']) && !empty($_GET['del1']) && !empty($_GET['del2']) && !empty($_GET['del3'])) //code to delete the selected Course
		{
			...
			if(unlink($createDeletePath))
			{
				...
				{
					echo '<script>alert("Deleted Successfully"); location.href="depcourse.php"</script>';
				}
				else
				{
					echo '<script>alert("Error while Deleting"); location.href="depcourse.php"</script>';
				}
			}
		}
		?>
