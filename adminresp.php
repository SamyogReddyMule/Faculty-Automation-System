	...
	while($fd = mysqli_fetch_array($fr))
		{
			...
			<tr>
				...
				<th><a style="text-decoration: none;" href="adminresp.php?upd=<?php echo $nam; ?>&upd1=<?php echo $nam1; ?>&upd2=<?php echo $nam2; ?>" id="upd" title="Edit Responsibility"> Y </a></th>
                <th><a style="text-decoration: none;" onclick="javascript: return confirm('Do you wnat to delete Responsibilities <?php echo $wv['cdescription']; ?>');" href="adminresp.php?del=<?php echo $nam; ?>&del1=<?php echo $nam1; ?>&del2=<?php echo $nam2; ?>" > X </a></th>
            </tr>
        <?php 
    	}
    }
    ...
    <?php
    	if(isset($_POST['add']))
		{
			...
			if(mysqli_num_rows(mysqli_query($con, $qsa2)) == 1)
			{
      							...
      							{
      								echo '<script>alert("Responsibility Added Successfully"); location.href="adminresp.php"</script>';
      							}
      							else
      							{
      								echo '<script>alert("Error while Adding"); location.href="adminresp.php"</script>';
      							}
  			if(mysqli_num_rows($qsar) == 1)
      			{
      				echo '<script>alert("Responsibility Already exist."); location.href="adminresp.php";</script>';
      			}
      		}
      		if(mysqli_num_rows(mysqli_query($con, $qsa2)) != 1)
      		{
      			echo '<script>alert("Faculty doesnt exist"); location.href="adminresp.php";</script>';
      		}
		}
		...
		if(isset($_POST['upd']))
		{
			...
			if(mysqli_num_rows(mysqli_query($con, $uqsa2)) == 1)
			{
				...
				{ 
					...
					if(mysqli_num_rows(mysqli_query($con, $sqlz)) != 1)
					{
										...
										{
											echo '<script>alert("Responsibility Details Updated Successfully"); location.href="adminresp.php"</script>';
										}
										else
										{
											echo '<script>alert("Error while updating"); location.href="adminresp.php"</script>';
										}
			...
			if(mysqli_num_rows(mysqli_query($con, $uqsa2)) != 1)
			{
				echo '<script>alert("Please check the faculty ID"); location.href="adminresp.php"</script>';
			}
		}
		if(!empty($_GET['del']) && !empty($_GET['del1']) && !empty($_GET['del2'])) //code to delete the selected Responsibility
		{
			...
			$getRow = mysqli_fetch_assoc($rsSelect);		
				...
				{
					echo '<script>alert("Deleted Successfully"); location.href="adminresp.php"</script>';
				}
				else
				{
					echo '<script>alert("Error while Deleting"); location.href="adminresp.php"</script>';
				}
