	...
	while($fd = mysqli_fetch_array($fr))
		{
			...
			<tr>
				...
				<th><a style="text-decoration: none;" href="depbooks.php?upd=<?php echo $nam; ?>&upd1=<?php echo $nam1; ?>&upd2=<?php echo $nam2; ?>" id="upd" title="Edit Paper Publication"> Edit</a></th>
                <th><a style="text-decoration: none;" onclick="javascript: return confirm('Do you wnat to delete Paper Publication <?php echo $wv["cname"]; ?>');" href="depbooks.php?del=<?php echo $nam; ?>&del1=<?php echo $nam1; ?>&del2=<?php echo $nam2; ?>" > Delete </a></th>
            </tr>
        <?php 
    	}
    }
    ...
    <?php
			...
			if(mysqli_num_rows($qsar) != 1)
			{
    			...
    			if ($fE == $afE)
    			{
      				...
      				if (!file_exists($dp))
      				{
      					...
      					{
      						$sql2 = "INSERT into certifications SET cfile='$nFN', cname='$cn', cindex='$ct', cjournal='$cs', cvol='$ce', cyear='$cy', ctype='Books', cuid='$ci'";
      						if (mysqli_query($con, $sql2))
      						{
      							echo '<script>alert("Paper Publication Added Successfully"); location.href="depbooks.php"</script>';
      						}
      						else
      						{
      							echo '<script>alert("Error while Adding"); location.href="depbooks.php"</script>';
      						}
      					}
      				}
      				if (file_exists($dp))
      				{
      					echo '<script>alert("File name Already exist. Please rename file and Try Again"); location.href="depbooks.php";</script>';
      				}
      			}
      			if($fE != $afE)
      			{
      				echo '<script>alert("File extension must be pdf"); location.href="depbooks.php";</script>';
      			}
      		}
      		if(mysqli_num_rows($qsar) == 1)
      		{
      			echo '<script>alert("Paper Publication Already exist."); location.href="depbooks.php";</script>';
      		}
      	}
      	if(mysqli_num_rows(mysqli_query($con, $qsa2)) != 1)
			{
				echo '<script>alert("Faculty Id is wrong or doesnt exists"); location.href="depbooks.php";</script>';
			}
		}
		...
		if(isset($_POST['upd']))
		{
			...
			{ 
				...
				if(mysqli_num_rows(mysqli_query($con, $sqlz)) != 1)
				{
    					...
    					if ($ufE == $uafE)
    					{
    						...
    						$unFN = "$ciu-$ucf";
      						...
      						if (!file_exists($udp))
      						{
      							...
      							{
									$wu12 = "UPDATE certifications SET cfile='$unFN', cname='$ucn', cindex='$uct', cjournal='$ucs', cvol='$uce', cyear='$ucy', cuid='$ciu' where cuid='$uci' and cname='$uci1' and ctype='Books' and cyear='$uci2' ";
									if (mysqli_query($con, $wu12))
									{
										echo '<script>alert("Paper Publication Details Updated Successfully"); location.href="depbooks.php"</script>';
									}
									else
									{
										echo '<script>alert("Error while updating"); location.href="depbooks.php"</script>';
									}
								}
							}
							if (file_exists($udp))
					...
					if($_FILES['ucf']['error'] == UPLOAD_ERR_NO_FILE)
					{
						$wu21 = "UPDATE certifications SET cname='$ucn', cindex='$uct', cjournal='$ucs', cvol='$uce', cyear='$ucy', cyear='$ucy', cuid='$ciu' where cuid='$uci', cuid='$uci' and cname='$uci1' and ctype='Books' and cyear='$uci2' ";
			...
			if($ciu == $uci && $ucn == $uci1 && $ucy == $uci2)
			{
    				...
    				if ($ufE == $uafE)
    				{
						...
						if (file_exists($udp))
      					...
      					if (!file_exists($udp))
      					{
       						if(move_uploaded_file($ufT, $udp)) 
      						{
								$wu1 = "UPDATE certifications SET cfile='$unFN', cname='$ucn', cindex='$uct', cjournal='$ucs', cvol='$uce', cyear='$ucy', cuid='$ciu' where cuid='$uci' and cname='$uci1' and ctype='Books' and cyear='$uci2' ";
				...
				if($_FILES['ucf']['error'] == UPLOAD_ERR_NO_FILE)
				{
					$wu2 = "UPDATE certifications SET cname='$ucn', cindex='$uct', cjournal='$ucs', cvol='$uce', cyear='$ucy', cuid='$ciu' where cuid='$ciu' and cname='$uci1' and ctype='Books' and cyear='$ucy' ";
		...
		if(!empty($_GET['del']) && !empty($_GET['del1']) && !empty($_GET['del2'])) //code to delete the selected Paper Publication
		{
			...
			if(unlink($createDeletePath))
			{
				...
				{
					echo '<script>alert("Deleted Successfully"); location.href="depbooks.php"</script>';
				}
				else
				{
					echo '<script>alert("Error while Deleting"); location.href="depbooks.php"</script>';
				}
			}
		}
		?>
