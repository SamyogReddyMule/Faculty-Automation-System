	...
	<?php
			...
			while($ld = mysqli_fetch_array($lr))
			{
				...
				?>
					<tr>
						...
						<th><a style="text-decoration: none;" href="depleaves.php?upd0=<?php echo $a; ?>&upd1=<?php echo $a1; ?>&upd2=<?php echo $a2; ?>"> Y </a></th>
						<th><a style="text-decoration: none;" onclick="javascript: return confirm('Do you want to delete the leave');" href="depleaves.php?del0=<?php echo $a; ?>&del1=<?php echo $a1; ?>&del2=<?php echo $a2; ?>&del3=<?php echo $a3; ?>"> X </a></th>
               		</tr>
            <?php
      		}
      	}
	 ...
	 <form class="f1" id="f1" action="" method="POST" style="display: none" enctype="multipart/form-data">
    	...
    	if(isset($_POST['add']))
		{
			...
			$cs = $_POST['cs'];
			$ce = $_POST['ce'];
			$ca1 = (date_diff(date_create($cs), date_create($ce)))->format("%a");
			$ca = intval($ca1) + 1;
			...
			{
			$sql2 = " SELECT *FROM leaves where luid='$ci' and lyear='$cy' and lstart='$cs' or lend='$ce' ";
			if(mysqli_num_rows(mysqli_query($con, $sql2)) != 1)
			{

				$edit = "SELECT lremain, lstart FROM leaves where lyear='$cy' and lstart > '$cs' ";
				$er = mysqli_query($con, $edit);
				while($uder = mysqli_fetch_array($er))
				{
					$g1 = $er['lremain'];
					$g2 = $er['lstart'];
					$g3 = $cr1 - $g1;
					mysqli_query($con, "UPDATE leaves SET lremain='$g3' where luid='$ci' and lyear='$cy' and lstart='$g2' ");
				}
				$add = "INSERT INTO leaves set luid='$ci', lyear='$cy', lrason='$cl', lstart='$cs', lend='$ce', lremain='$cr', ltime='$ca' ";
				if(mysqli_query($con, $add))
				{
					echo '<script>alert("Inserted Successfully");location.href="depleaves.php";</script>';
				} else {
					echo '<script>alert("Error while inserting"); location.href="depleaves.php";</script>';
				}
			}
			if(mysqli_num_rows(mysqli_query($con, $sql2)) == 1)
			{
				echo '<script>alert("Leave details already in Database"); location.href="depleaves.php";</script>';
			}
		}
			if(mysqli_num_rows(mysqli_query($con, $qsa2)) != 1)
			{
				echo '<script>alert("Faculty doesnt seem to care!"); location.href="depleaves.php";</script>';
			}
		}
		...
		if(isset($_POST['change']))
		{
			...
			$ucs = $_POST['ucs'];
			$uce = $_POST['uce'];
			$uca1 = (date_diff(date_create($ucs), date_create($uce)))->format("%a");
			$uca = intval($uca1) + 1;
			...
			$uqsa2 = "SELECT * FROM faculty_details where fuid = '$uci' ";
			...
			if($uci != $x || $ucs != $x1)
			{
				...
				if(mysqli_num_rows(mysqli_query($con, $sql6)) != 1)
				{
					...
					{
						echo '<script>alert("Updated Successfully"); location.href="depleaves.php";</script>';
					} else {
						echo '<script>alert("Error while Updating"); location.href="depleaves.php";</script>';
					}
				}
				if(mysqli_num_rows(mysqli_query($con, $sql6)) == 1)
				{
					echo '<script>alert("Leave Already exists"); location.href="depleaves.php";</script>';
				}
			}
			if($uci == $x && $ucs == $x1)
			{
		...
		if(mysqli_num_rows(mysqli_query($con, $uqsa2)) == 1)
			{
				echo '<script>alert("Faculty id doesnt found in the database"); location.href="depleaves.php";</script>';
			}
		}
		if(!empty($_GET['del0']) && !empty($_GET['del1']) && !empty($_GET['del2']))
		{
			...
			if(mysqli_query($con, $dadd))
			{
				...
				location.href = "depleaves.php"; </script>';
			}else{
