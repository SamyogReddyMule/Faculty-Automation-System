...
<html lang="en">
    <head>
        ...
        <title> MBA Admin Login Page</title>
    </head>
    <body>
    <?php
        session_start();
        ...
        if(isset($_POST['login']))
            {
                ...
                if($duid !== $u['duid'] || $dpassword !== $u['dpwd'] )
                {
                    echo '<script>alert("Unique ID or Password doesnt match"); location.href="mbaadminlogin.php"</script>';
                }
            }
        ?>
    <form class="f1" id="f1" action="" method="POST" autocomplete="off">
        <h3 class="h2"> MBA Admin Login </h3>
