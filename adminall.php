	...
	<?php
		...
		while($fd = mysqli_fetch_array($fr))
		{
      		...
      		while($pd = mysqli_fetch_array($wr5))
			{
			  ...
			  echo '<tr> <th colspan="6" style="background-color: grey;"> Books Published </th> </tr>';
			  $w6 = "SELECT * FROM certifications where ctype = 'Books' and cuid= '$uidi' ";
			  $wr6 = mysqli_query($con, $w6);
				while($bd = mysqli_fetch_array($wr6))
			  {
					  echo '<tr>
							  <td> '.$bd["cname"].' </td>
							  <td> '.$bd["cyear"].' </td>
							  ...
							  <td> '.$bd["cvol"].' </td>
							  <td> '.$bd["cindex"].' </td>
							  <td><a href="certs/'.$bd["cfile"].'" target="_blank"> '.$bd["cfile"].' </td>
							 </tr>';
				}
      		echo '</table>';
		}
