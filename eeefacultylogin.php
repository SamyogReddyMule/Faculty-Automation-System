...
<html lang="en" >
    <head>
        ...
        <title>EEE Faculty Login Page</title>
    ...
    <body >
        <div class="d2" style="justify-content: center;">
            ...
            session_start();
            include 'db.php';
            include 'eeeloginmenu.php';
            if(isset($_POST['login']))
            {
                $fuid = $_POST['uid'];
                $fpassword = $_POST['pass'];
                $_SESSION['id'] = $_POST['uid'];
                $sql = "SELECT *FROM faculty_details where fuid='$fuid' and fpwd='$fpassword' AND fdep='EEE'";
                ...
                {
                    echo '<script>alert("Unique ID or Password doesnt match"); location.href="eeefacultylogin.php"</script>';
                }
            }
            if(isset($_POST['change']))
            {
                ...
                if(mysqli_num_rows($r) == 1)
                {
                    if($npass == $cpass)
                    {
                        ...
                        {
                            echo '<script>alert("Password changed Successfully"); location.href="eeefacultylogin.php"</script>';
                        }
                    }
                    if($npass != $cpass)
                    {
                        echo '<script>alert("New Passwords are not same"); location.href="eeefacultylogin.php"</script>';
                    }
                }
                if(mysqli_num_rows($r) !== 1)
                {
                    echo '<script> alert("Unique Id or Date of Birth is incorrect"); location.href="eeefacultylogin.php"</script>';
                }
            }
            ?>
            <div class="d3" style="margin-top: 10px;">
                <form class="f1" id="f1" action="" method="POST" autocomplete="off">
                    <h3>EEE Faculty Login</h3>
