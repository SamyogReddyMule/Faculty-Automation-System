...
<body>
	...
	while($fd = mysqli_fetch_array($fr))
		{
			...
			<tr>
				...
				<th><a style="text-decoration: none;" href="adminmemb.php?upd=<?php echo $nam; ?>&upd1=<?php echo $nam1; ?>&upd2=<?php echo $nam2; ?>" id="upd" title="Edit Award"> Y </a></th>
                <th><a style="text-decoration: none;" onclick="javascript: return confirm('Do you wnat to delete Award <?php echo $wv['cname']; ?>');" href="adminmemb.php?del=<?php echo $nam; ?>&del1=<?php echo $nam1; ?>&del2=<?php echo $nam2; ?>" > X </a></th>
            </tr>
        <?php 
    	}
    }
    ...
    <?php
    	if(isset($_POST['add']))
		{
			...
			$ce = $_POST['ce'];
			$cf = $_FILES['cf']['name'];
			$qsa = "SELECT * FROM memberships where cuid = '$ci' and cname='$cn' and ctype='$cs' ";
				...
				if(mysqli_num_rows($qsar) != 1)
				{
    				...
    				if ($fE == $afE)
    				{
      					...
      					if (!file_exists($dp))
      					{
      						...
      						{
      							$sql2 = "INSERT into memberships SET cfile='$nFN', cname='$cn', ctype='$cs', cnumber='$ce', cuid='$ci'";
      							if (mysqli_query($con, $sql2))
      							{
      								echo '<script>alert("Membership Added Successfully"); location.href="adminmemb.php"</script>';
      							}
      							else
      							{
      								echo '<script>alert("Error while Adding"); location.href="adminmemb.php"</script>';
      							}
      						}
      					}
      					if (file_exists($dp))
      					{
      						echo '<script>alert("File name Already exist. Please rename file and Try Again"); location.href="adminmemb.php";</script>';
      					}
      				}
      				if($fE != $afE)
      				{
      					echo '<script>alert("File extension must be pdf"); location.href="adminmemb.php";</script>';
      				}
      			}
      			if(mysqli_num_rows($qsar) == 1)
      			{
      				echo '<script>alert("Membership Already exist."); location.href="adminmemb.php";</script>';
      			}
      		}
      		if(mysqli_num_rows(mysqli_query($con, $qsa2)) != 1)
      		{
      			echo '<script>alert("Faculty doesnt exist"); location.href="adminmemb.php";</script>';
      		}
		}
		if(!empty($_GET['upd']) && !empty($_GET['upd1']) && !empty($_GET['upd2']))
		{
			...
			$uci1 = $_GET['upd1'];
			$uci2 = $_GET['upd2'];
			$sql1 = "SELECT * FROM memberships where cuid='$uci' and cname = '$uci1' and ctype='$uci2'  ";
			...
			<h3 class="h3">Update Membership</h3>
	        ...
	        <label class="l1"> Membership Type </label>
	        <input class="i1" type="text" name="ucs" value="'.$wud["ctype"].'" required> <br>

	        <label class="l1" > Membership Number </label>
	        <input class="i1" type="text" name="uce" value="'.$wud["cnumber"].'" required> <br>
		...
		if(isset($_POST['upd']))
		{
			...
			if(mysqli_num_rows(mysqli_query($con, $uqsa2)) == 1)
			{
				...
				{ 
					$sqlz = "SELECT * FROM memberships where cuid='$ciu' and cname = '$ucn'  and cnumber='$uce' and ctype='$ucs' ";
					if(mysqli_num_rows(mysqli_query($con, $sqlz)) != 1)
					{
    						...
    						if ($ufE == $uafE)
    						{
    							...
    							$unFN = "$ciu-$ucf";
      							...
      							if (!file_exists($udp))
      							{
      								...
      								{
										$wu12 = "UPDATE memberships SET cfile='$unFN', cname='$ucn',  ctype='$ucs', cnumber='$uce', cuid='$ciu' where cuid='$uci' and cname='$uci1'  and ctype='$uci2' ";
										if (mysqli_query($con, $wu12))
										{
											echo '<script>alert("Membership Details Updated Successfully"); location.href="adminmemb.php"</script>';
										}
										else
										{
											echo '<script>alert("Error while updating"); location.href="adminmemb.php"</script>';
										}
									}
								}
								if (file_exists($udp))
						...
						if($_FILES['ucf']['error'] == UPLOAD_ERR_NO_FILE)
						{
							$wu21 = "UPDATE memberships SET cname='$ucn',  ctype='$ucs', cnumber='$uce',  cuid='$ciu' where cuid='$uci' and cname='$uci1' and ctype='$uci2' ";
				...
				if($ciu == $uci && $ucn == $uci1 && $ucs == $uci2)
				{
    					...
    					if ($ufE == $uafE)
    					{
							...
							if (file_exists($udp))
      						...
      						if (!file_exists($udp))
      						{
       							if(move_uploaded_file($ufT, $udp)) 
      							{
									$wu1 = "UPDATE memberships SET cfile='$unFN', cname='$ucn', ctype='$ucs', cnumber='$uce',  cuid='$ciu' where cuid='$uci' and cname='$uci1'  and ctype='$uci2' ";
					...
					if($_FILES['ucf']['error'] == UPLOAD_ERR_NO_FILE)
					{
						$wu2 = "UPDATE memberships SET cname='$ucn',  ctype='$ucs', cnumber='$uce',  cuid='$ciu' where cuid='$ciu' and cname='$uci1'  and ctype='$uci2' ";
			...
			if(mysqli_num_rows(mysqli_query($con, $uqsa2)) != 1)
			{
				echo '<script>alert("Please check the faculty ID"); location.href="adminmemb.php"</script>';
			}
		}
		if(!empty($_GET['del']) && !empty($_GET['del1']) && !empty($_GET['del2'])) //code to delete the selected award
		{
			...
			$m1 = $_GET['del1'];
			$m2 = $_GET['del2'];
			$selectSql = "SELECT * from memberships where cuid='$m' and cname = '$m1'  and ctype='$m2' ";
			...
			{
				$sqle = "DELETE FROM memberships where cuid='$m' and cname = '$m1'  and ctype='$m2' ";
				if(mysqli_query($con, $sqle))
				{
					echo '<script>alert("Deleted Successfully"); location.href="adminmemb.php"</script>';
				}
				else
				{
					echo '<script>alert("Error while Deleting"); location.href="adminmemb.php"</script>';
				}
			}
		}
		?>
