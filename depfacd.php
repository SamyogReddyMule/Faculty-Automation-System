...
<body>
	<?php
		...
		<?php
			$f = "SELECT * FROM faculty_details where fdep='$dep'";
		...
		<tr>
			...
			<td> <?php echo $fd["fdep"]; ?> </td>
			<td> <?php echo $fd["fphn"]; ?> </td>
			<td> <?php echo $fd["femail"]; ?> </td>
			<th><a style="text-decoration: none;" href="depfacd.php?upd0=<?php echo $a; ?>"> Update </a></th>
			<th><a style="text-decoration: none;" onclick="javascript: return confirm('Do you want to delete the faculty <?php echo $fd["fname"];?>(<?php echo $fd["fuid"];?>)');" href="depfacd.php?del0=<?php echo $a; ?>"> Delete</a></th>
        </tr>
        <?php
          	}
      	?>
	 ...
	 <form class="f1" id="f1" action="" method="POST" style="display: none" enctype="multipart/form-data">
        ...
        <input class="i1" type="text" placeholder="Enter Department" name="cd" required> <br>
    	...
    	if(isset($_POST['add']))
		{
			...
			if(mysqli_num_rows(mysqli_query($con, $sqlz)) != 1)
			{
				...
				$add1 = " INSERT INTO experience set expid='$ci', ename='Sir C R Reddy College of Engineering', eeyear= '$cw', edes='$cs' ";
       	 		mysqli_query($con, $add1);
       	 		if(mysqli_query($con, $add))
				{
					echo '<script>alert("Inserted Successfully"); location.href="depfacd.php";</script>';
				}else{
					echo '<script>alert("Error while inserting"); location.href="depfacd.php";</script>';
				}
			}
			if(mysqli_num_rows(mysqli_query($con, $sqlz)) == 1)
			{
				echo '<script>alert("Faculty already Exists"); location.href="depfacd.php";</script>';
			}
		}
		if(isset($_GET['upd0']) && $_GET['upd0'])
		{
			...
			<label class="l1" > Department </label>
	        <input class="i1" type="text" value="'.$wud["fdep"].'" placeholder="Enter Department" name="ucd" required> <br>
		...
		if(isset($_POST['change']))
		{
			...
			if($uci != $x)
			{
				...
				{
					$uadd = "UPDATE faculty_details set fuid='$uci', fname='$ucn', fdob='$ucy', fgen='$ucl', fdes='$ucs', fdep='$ucd', fphn='$uce', femail='$ucf', fage='$uca' where fuid='$x' ";
       	 			if(mysqli_query($con, $uadd))
					{
						echo '<script>alert("Updated Successfully"); location.href="depfacd.php";</script>';
					}
					else
					{
						echo '<script>alert("Error while Updating"); location.href="depfacd.php";</script>';
					}
				}
				if(mysqli_num_rows(mysqli_query($con, $usqlz)) == 1)
				{
			...
			if($uci == $x)
			{
       	 		$uadd = "UPDATE faculty_details set fuid='$uci', fname='$ucn', fdob='$ucy', fgen='$ucl', fdes='$ucs',fdep='$ucd', fphn='$uce', femail='$ucf' where fuid='$x' ";
       	 		if(mysqli_query($con, $uadd))
				{
					echo '<script>alert("Updated Successfully"); location.href="depfacd.php";</script>';
				}
		...
		if(isset($_GET['del0']) && $_GET['del0'])
		{
			...
			$cdelr = mysqli_query($con, $cdel);
			while ($cdeld = mysqli_fetch_array($cdelr)) {
				unlink("certs/".$cdeld['cfile']);
			}
			$cdel1 = "SELECT wfile FROM workload where wuid='$b' ";
			$cdelr1 = mysqli_query($con, $cdel1);
			while ($cdeld1 = mysqli_fetch_array($cdelr1)) {
			...
			{
				echo '<script>alert("Deleted Successfully"); location.href="depfacd.php"</script>';
			}
			else
			{
				echo '<script>alert("Error while Deleting"); location.href="depfacd.php"</script>';
			}
		}
    ?>
</body>
